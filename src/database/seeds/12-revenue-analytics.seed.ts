    ...
    for (const day of days) {
      ...
      if (isAnomaly) {
        if (Math.random() > 0.5) {
          ...
          console.log(
            `[SEED] Surge anomaly on ${day.toISOString().split('T')[0]}`,
          );
        } else {
          baseOrders = 0; // Drop anomaly
          console.log(
            `[SEED] Drop anomaly on ${day.toISOString().split('T')[0]}`,
          );
        }
      }

      const orderCount =
        isAnomaly && baseOrders === 0
          ? 0
          : Math.floor(Math.random() * baseOrders) + 1;
