...
export async function seedBulkCourses(dataSource: DataSource) {
  ...
  if (!instructor) {
    instructor = await userRepo.findOne({ where: { role: { roleName: 'INSTRUCTOR' } }, relations: ['role'] });
  }

  const allCategories = await categoryRepo.find({ where: { parentId: IsNull() } }); // Hoặc lấy tất cả không phân biệt root. Tùy ý.
  ...
  if (!instructor || validCategories.length === 0) {
    console.log('Bỏ qua bulk seed: Không đủ dữ liệu instructor hoặc categories.');
    return;
  }
  ...
  const categoryMap: Record<string, { topics: string[], images: string[] }> = {
    'lap-trinh': {
      topics: ['Fullstack Web với Next.js 15', 'Python cho Data Science', 'Lập trình Go cơ bản', 'Microservices với NestJS', 'React Native thực chiến', 'Bảo mật Web nâng cao'],
      images: [
    ...
    'kinh-doanh': {
      topics: ['Khởi nghiệp Tinh gọn (Lean Startup)', 'Kỹ năng bộ máy Lãnh đạo', 'Nghệ thuật Đàm phán triệu đô', 'Quản trị chuỗi cung ứng', 'Phát triển mô hình SaaS'],
      images: [
    ...
    'tai-chinh-ke-toan': {
      topics: ['Đầu tư Chứng khoán từ con số 0', 'Kế toán Thuế chuyên sâu', 'Phân tích Báo cáo Tài chính', 'Tiền điện tử và Blockchain'],
      images: [
        'https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=800&q=80',
        'https://images.unsplash.com/photo-1611974714851-eb6053e6c3e1?w=800&q=80',
      ]
    },
    'thiet-ke': {
      topics: ['Thiết kế UI/UX với Figma', 'Masterclass Adobe Photoshop', 'Thiết kế 3D bằng Blender', 'Typography trong Branding'],
      images: [
    ...
    'marketing': {
      topics: ['SEO Nâng cao (Search Engine Optimization)', 'Quảng cáo Facebook & Google chuyên sâu', 'Content Marketing đỉnh cao', 'Influencer Marketing'],
      images: [
    ...
    'phat-trien-ban-than': {
      topics: ['Quản lý thời gian hiệu quả', 'Kỹ năng Giao tiếp tự tin', 'Tư duy Sáng tạo đột phá', 'Xây dựng thương hiệu cá nhân'],
      images: [
    ...
    'suc-khoe': {
      topics: ['Yoga phục hồi năng lượng', 'Gym tại nhà không cần dụng cụ', 'Chế độ ăn Eat Clean khoa học', 'Thiền định giảm Stress'],
      images: [
    ...
    'am-nhac': {
      topics: ['Guitar đệm hát cơ bản', 'Tự học Piano tại gia', 'Làm nhạc EDM trên FL Studio', 'Hát Karaoke như ca sĩ'],
      images: [
    ...
    'nhiep-anh-video': {
      topics: ['Nhiếp ảnh Chân dung ngoài trời', 'Quay phim quảng cáo chuyên nghiệp', 'Edit video bằng Premiere Pro', 'Bay Flycam cơ bản'],
      images: [
    ...
    'ngon-ngu': {
      topics: ['Tiếng Anh giao tiếp công sở', 'Luyện thi JLPT N3 cấp tốc', 'Tiếng Hàn giao tiếp hàng ngày', 'Tiếng Trung 4 kỹ năng'],
      images: [
  ...
  const prefixes = ['Mastering', 'Cơ bản về', 'The Complete', 'Thực hành', 'Nhập môn', 'Chuyên sâu', 'Bí mật của', 'Học nhanh'];
  const suffixes = ['từ Zero đến Hero', 'cho người mới', 'Masterclass', 'thực chiến', 'trong 30 ngày'];

  const generatedSlugs = new Set();
  const bulkCourses: Partial<Course>[] = [];

  // Lấy list root categories để map
  const rootCategories = await categoryRepo.find({ where: { parentId: IsNull() } });

  for (let i = 0; i < 40; i++) {
    const subCategory = validCategories[Math.floor(Math.random() * validCategories.length)];
    const parentCategory = rootCategories.find(rc => rc.id === subCategory.parentId);
    
    const config = categoryMap[parentCategory?.slug || 'lap-trinh'] || categoryMap['lap-trinh'];
    ...
    bulkCourses.push({
      ...
      publishedAt: new Date(Date.now() - Math.floor(Math.random() * 30 * 24 * 60 * 60 * 1000)), // Trong 30 ngày gần đây
    });
  }
  ...
  console.log(`Bắt đầu tạo Sections & Lessons cho ${bulkCourses.length} khóa học...`);
  ...
  console.log(`Đã gỡ bỏ data random cũ và seed mới ${bulkCourses.length} khóa học (kèm đầy đủ Sections/Lessons) cho instructor UID ${instructor.id}`);
}
