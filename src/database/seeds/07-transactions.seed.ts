...
export async function seedTransactions(dataSource: DataSource) {
  ...
  if (!course) {
    console.log(
      'Không tìm thấy khóa học Master Linux, bỏ qua seed transaction.',
    );
    return;
  }
  ...
  let instructorWallet = await walletRepo.findOne({
    where: { user_id: instructorId },
  });
  if (!instructorWallet) {
    instructorWallet = await walletRepo.save(
      walletRepo.create({
  ...
  let studentWallet = await walletRepo.findOne({
    where: { user_id: studentId },
  });
  if (!studentWallet) {
    studentWallet = await walletRepo.save(
      walletRepo.create({
        user_id: studentId,
        balance_pending: 0,
        balance_available: 0,
  ...
  instructorWallet.balance_pending =
    Number(instructorWallet.balance_pending) + instructorAmount;
  instructorWallet.total_earned =
    Number(instructorWallet.total_earned) + instructorAmount;
  ...
  await transactionRepo.save(
    transactionRepo.create({
      ...
      status: 'LOCKED',
      balance_after:
        Number(instructorWallet.balance_available) +
        Number(instructorWallet.balance_pending),
  ...
  systemWallet.balance_available =
    Number(systemWallet.balance_available) + platformFee;
  ...
  let currentSystemBalance =
    Number(systemWallet.balance_available) +
    Number(systemWallet.balance_pending);
  let currentInstructorBalance =
    Number(instructorWallet.balance_available) +
    Number(instructorWallet.balance_pending);
  for (let i = 1; i <= 20; i++) {
    const amount = 50000 + Math.floor(Math.random() * 500000);
    const type =
      i % 3 === 0 ? 'EARNING' : i % 3 === 1 ? 'PLATFORM_FEE' : 'PURCHASE';
